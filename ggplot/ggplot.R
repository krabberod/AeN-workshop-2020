### ggplot
# References and templates for this exercise can be found here http://r-statistics.co/Complete-Ggplot2-Tutorial-Part1-With-R-Code.html and http://r-statistics.co/ggplot2-Tutorial-With-R.html
...
oslo_fjord <- readRDS(file.path("stats", "OsloFjord_phyloseq.rds"))
metadata <- sample_data(oslo_fjord)
colnames(metadata)

# Init ggplot
ggplot(metadata, aes(x = Date, y = temp))  # Date and temp are columns in 'metadata'
...
ggplot(metadata, aes(x = Date, y = temp)) + geom_point()
...
g <- ggplot(metadata, aes(x = Date, y = temp)) + geom_point() + geom_smooth(method="lm")  # set se=FALSE to turnoff confidence bands
...
g1 + labs(title="Water temperature in Oslo Fjord", subtitle="in 2016 and 2017", y="Temperature", x="Day of the Year", caption="CTD")

# or

g1 + ggtitle("Water temperature in Oslo Fjord", subtitle="in 2016 and 2017") + xlab("Date") + ylab("Temperature")
...
ggplot(metadata, aes(x = DoY, y = temp)) +
  geom_point(col="steelblue", size=3) +   # Set static color and size for points
  geom_smooth(method="lm", col="firebrick") +  # change the color of line
  coord_cartesian(xlim=c(0, 365), ylim=c(0, 20)) +
  labs(title="Water temperature in Oslo Fjord", subtitle="in 2016 and 2017", y="Temperature", x="Day of the Year", caption="CTD")
...
# Change breaks
gg + scale_x_continuous(breaks=seq(0, 50, 10))
...
gg + scale_x_continuous(breaks=seq(0, 50, 10), labels = letters[1:9])
